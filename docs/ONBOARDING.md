...
5. **Legal pages** — `/terms` and `/privacy` load without login (also reachable from register checkbox). Privacy must name Twilio, say mobile numbers are not shared for marketing, note message frequency, and include “message and data rates may apply.”
