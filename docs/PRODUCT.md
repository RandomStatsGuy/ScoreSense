...
- ScoreSense-only leagues persist weekly lineups on This Week and score weeks with configurable ScoreSense PPR (full PPR by default) (nflverse; internal id `hub_ppr` — the string "Hub PPR" never reaches UI). Commissioners can calculate during the NFL week; a calculate after the slate ends locks that week's lineups. Linked Sleeper leagues still set and score lineups in Sleeper; Game center reads Sleeper.
...
Game center gives native commissioners Calculate week / Recalculate week controls after the draft. Calculate can run during the NFL week when stats are available: it saves current player/team totals and a scoring-rules snapshot, updates standings, and leaves leftover lineup edits open. Recalculate refreshes those live scores. A calculate after the last game locks that week's lineups. Recalculation confirms that it replaces the selected week's totals using current saved rules. Saving scoring settings alone never rewrites results; Game center flags an older rules snapshot. Native scoring is commissioner-triggered, not an automatic live feed.
