...
Home names the manager’s roster hole over a commissioner invite when both are due. Gate hero copy on load: skeleton or “Checking what is due…” until the payload lands — never a confident headline over unresolved data. After 3s of a long load, say the sync is still working. This Week hero copy comes from board state (loading / error / empty pre-draft) — never “No swap worth making” over an error. The hero is the one place that states the lineup-call count — do not repeat it in At a glance and the rail note. Empty starter slots say **Empty**; “Waiting on roster” is the unresolved-roster rail, not a loading chip. Game center’s unscored placeholder says **No scores yet**, not Waiting. Pre-draft Game center names draft night and when scores start; it does not invent a 1–10 seed from 0–0 or play last year's Sleeper week as this week's scores. Home and Game center read the same standings payload — last-season records stay labeled Last season. Sync league is the routine strip action; New league is a quieter create control, not a matching ghost button. Cap and My team read one dead-cap story from `rosterFormat.js`. Contract field values never repeat their own labels. On My team the cap card leads with leftover for draft; committed is the footnote. Dead cap is a tooltip or inline · $N dead, not a legend that outweighs the number. The roster table uses page scroll — no inner max-height scrollbar. Needs attention says **Cap** for over-cap, not Cap planner, and **Review extensions** to filter My team. Staff-only Remove is gated, confirmed, and visually separated from Queue extension. The contract dialog is `aria-modal` and moves focus to the heading. Document title matches the UI: **My team**. Route-level nav is links whose accessible name is the visible label; descriptions sit on `title` / `aria-description`. The league bar live region is only the Needs attention line. Draft seating chips count claimed teams. Locked draft night renders in the viewer’s timezone with an abbreviation; Draft setup names league time once. Rosters defaults to the league-wide Overpay/Bargain list; the manager rail is the drill-down. Contract judgment is the word alone when the dollar delta is zero. Expire chips say Extendable, never a question that looks like a control.
