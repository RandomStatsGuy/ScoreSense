...
**Tools:** DFS · Mock draft · Best ball. DFS classic and season-long: Vegas games are the board. Pick a shootout to lock QB +2 and a bring-back from that game, then build the leftover under the cap. Highest is a corner tag, not a reason by itself. Captain modes stay one-game CPT / MVP. Mock draft field size follows the linked league when matching that league's rules. Recent mocks live on the launch rail. A practice room has no Drop or Trade. Simulate uses the same bot pricing as the live auction, shows N of the remaining pool, and never disables Discard. Never say Available players — the rail eyebrow is Player pool.
...
Commissioner Drop on Contracts and My team removes a player with no dead cap so staff can add them to another team before the draft. After draft it writes immediately — no pending tray. Cut is the penalty path. After draft, owners Cut on My team and commissioners Cut on Roster management · Contracts; those writes are immediate. Dead cap hits only the season the player is cut — later years of a multi-year deal free in full.
