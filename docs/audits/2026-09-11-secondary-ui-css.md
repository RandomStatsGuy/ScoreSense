...
Compared with the build containing the cached Best ball / parallel startup bundle (#441).

| Startup asset | Before | After | Reduction |
| --- | ---: | ---: | ---: |
| Entry JavaScript, minified | 576.59 KB | 455.00 KB | 21.1% |
...
| Service-worker precache | 2,523.43 KiB / 66 entries | 744.84 KiB / 10 entries | 70.5% fewer bytes |
...
- Frontend tests: 722/724 passed. The same existing availability-effect CRLF assertion and null typical-miss formatting failures remain.
...
- The layout audit reported the same 94 findings in both fixture builds; none were introduced by this change.
