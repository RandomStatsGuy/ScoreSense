"""Daily prewarm of draft-room fantasy media digests (LLM when configured)."""
...
def prewarm_fantasy_media_digests(
    *,
    ...
    """
    Build/cache fantasy-show media digests for mention-rich draft-pool players.
    ...
    if "player_id" in pool.columns:
        pool_ids = set(
            pool.sort_values("Season Proj", ascending=False)
            .head(pool_cap)["player_id"]
            .astype(str)
        )
...
# Backward-compatible alias for older job imports (not an API field alias).
prewarm_beat_digests = prewarm_fantasy_media_digests


if __name__ == "__main__":
    import json

    print(json.dumps(prewarm_fantasy_media_digests(), indent=2))
