...
from src.config import BACKTEST_DIR, DEFAULT_ETL_SEASONS, DEFAULT_TEST_SEASONS, DEFAULT_TRAIN_SEASONS, MODEL_DIR, PROCESSED_DATA_DIR, PROJECT_ROOT
...
with tab_projections:
    ...
    season = col2.number_input("Season", min_value=2018, max_value=2026, value=max(DEFAULT_TEST_SEASONS))
...
with tab_pipeline:
    ...
    seasons = st.text_input(
        "Seasons (space-separated)",
        " ".join(str(s) for s in DEFAULT_ETL_SEASONS),
    )
    ...
    if st.button("3. Run walk-forward backtest"):
        with st.spinner("Running backtest..."):
            metrics = run_all_backtests(test_seasons=DEFAULT_TEST_SEASONS)
            st.json(metrics)
