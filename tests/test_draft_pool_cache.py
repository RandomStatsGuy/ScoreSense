...
def test_pool_fingerprint_includes_rookie_overrides(monkeypatch, tmp_path):
    feat = tmp_path / "qb_mlready.parquet"
    feat.write_bytes(b"x")
    overrides = tmp_path / "rookie_role_overrides.yaml"
    overrides.write_text("2026: []\n", encoding="utf-8")
    monkeypatch.setattr(draft_pool_cache, "PROCESSED_DATA_DIR", tmp_path)
    monkeypatch.setattr(draft_pool_cache, "MODEL_DIR", tmp_path)

    import src.config as config

    monkeypatch.setattr(config, "ROOKIE_ROLE_OVERRIDES_PATH", overrides)
    fp1 = draft_pool_cache.pool_fingerprint()
    overrides.write_text("2026:\n  - player: Test\n    role_mult: 2.0\n", encoding="utf-8")
    fp2 = draft_pool_cache.pool_fingerprint()
    assert fp1 != fp2
