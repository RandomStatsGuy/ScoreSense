...
def test_api_sms_opt_in_persists_on_account(api_client, mock_send):
    ...
    assert ok.status_code == 200
    assert ok.json()["user"]["sms_opt_in"] is True
    assert ok.json()["user"]["phone"] == "5551234567"
    me = api_client.get("/api/auth/me", headers=headers)
    assert me.status_code == 200
    assert me.json()["user"]["sms_opt_in"] is True
