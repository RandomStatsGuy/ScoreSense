...
def test_auction_win_contracts_rookie_and_vet_stay_flat():
    ...
    assert vet["veteran_salary_static"] is True
    assert [y["salary"] for y in vet["schedule"]] == [39, 39]
    assert float(vet.get("step_up_per_year") or 0) == 0
...
def test_auction_win_contracts_follow_commissioner_term_and_rookie_salary_rules():
    ...
    assert vet["veteran_salary_static"] is False
...
def test_contract_renewal_permissions_follow_league_rules():
    ...
    assert can_renew(rookie, disabled_rookie_rules) == (
        False,
        "Rookie deal extensions are disabled by league rules.",
    )

    assert can_renew(veteran, rules)[0] is True
    disabled_vet_rules = rules.model_copy(update={
        "contracts": rules.contracts.model_copy(update={"allow_veteran_renewal": False}),
    })
    ok, msg = can_renew(veteran, disabled_vet_rules)
    assert ok is False
    assert "Vet deal" in msg
...
def test_veteran_contract_defaults_to_two_stepped_years():
    c = build_veteran_contract(10, step_up=5)
    assert c["years_remaining"] == 2
    assert [y["salary"] for y in c["schedule"]] == [10, 15]
...
def test_roster_edit_first_term_stays_flat_extension_steps():
    ...
    assert float(vet.get("step_up_per_year") or 0) == 0
    assert vet.get("veteran_salary_static") is True
