...
import { joinLandingPath, joinLandingSearch, withLocationSearch } from "./redirectSearch.js";
...
test("invite and claim links land on Draft", () => {
  ...
  assert.equal(
    withLocationSearch(joinLandingPath("?invite=tok"), joinLandingSearch("?invite=tok"), ""),
    "/hub/draft?invite=tok",
  );
  assert.equal(joinLandingSearch("?invite=tok&pos=qb&week=1"), "?invite=tok");
  assert.equal(joinLandingSearch("?claim=abc&invite="), "?claim=abc");
});
