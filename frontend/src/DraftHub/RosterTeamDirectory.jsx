...
export default function RosterTeamDirectory({ blocks, myTeamId, onChoose, headingRef }) {
  ...
  return <div className="rosters-team-directory">
    <div className="rosters-directory-heading"><div><h2 ref={headingRef} tabIndex={-1}>{C.chooseTeam}</h2><p>{C.chooseTeamHelp}</p></div>
    ...
    <div className="rosters-team-grid">{matches.map(b => <button key={b.team.id} type="button" className="rosters-team-card" onClick={() => onChoose(b.team.id)} aria-label={C.openTeam(ownerLine(b.team))}>
      ...
      <span className="rosters-team-facts"><span>{C.playerCount(activeRoster(b).length)}</span><span>{C.capRoom}<strong>{rosterMoney(b.stats?.unspent)}</strong></span></span>
