...
export const LIVING_SURFACES = Object.freeze({
  ...
  "hub.vibes": S({
    ...
    doNot: "Reuse HubExperience*. Aura is a personal start weight, not a new accent or award gold. Desktop: card left, Vibe ranking and VA-projections right, both in view. Front card is week-vs-vibe, not a giant cutout. Bio is the one noun for the profile control — labeled Bio, neutral, not accent blue. Sit and Start are equal; Undo is a meta control, not a third option. Sit is not amber. One progress readout lives by the card. Week and vibe week compare; aura is a labeled 0–99 meter. One rate per player per calendar day. VA-projections are research; This Week lineup uses the board number. Empty slots say Empty with Find {POS}. Review on This Week stays hidden until a rating exists. When Hub lineups can write, Set this slate is the one primary and Review on This Week is ghost. Otherwise Review is the one primary on the ranking card. The left column lists today's reads or collapses. Do not scrape Wikipedia. Do not say site board. Do not mention swipe on a pointer desktop.",
  }),
  "hub.game": S({
    label: "Game center",
    chrome: "matchup",
