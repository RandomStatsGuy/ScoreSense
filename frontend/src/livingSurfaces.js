...
export const LIVING_SURFACES = Object.freeze({
  ...
  "hub.room.live": S({
    ...
    also: [
      ...
      "frontend/src/DraftHub/DraftPlayerRail.jsx",
      "frontend/src/DraftHub/draftNominationTax.js",
      "frontend/src/DraftHub/draftNominationPresentation.js",
    ],
    doNot: "Do not wrap a live draft in HubExperience two-column settings chrome. Seats inherit DraftSeat from idle Draft and Mock. Never say Available players — the rail eyebrow is Player pool. A nomination queue stays on the viewer's board only — never on other teams in room state. Empty nomination keeps leftover and the viewer's queue on a right rail beside the pool — not a full-width stack. Do not hide Nominate when paused; disable it. Offline conduct hides clocks and Records a win instead of nominating a timer. Command bar leads with the job; connection is a quiet mark. Auction awards play on the block card (bid pulse, draining clock, 1s SOLD hold) — do not toast a sold result in the header. Hide empty fantasy narrative; a real line is the name tagline. High bid is never gold: blue while winning, primary text otherwise. Bots use locker marks and persona names, never identical emoji robots. Filled position chips are teal, over-max amber, empty muted. Won roster rows pop in like lockers. Simulate pins the block-card layout. Need vs Tax stays on the existing player rail — do not fork a second pool. Tax is advice only and never broadcasts intent. Owner name labels the rival; nickname is never the only label.",
  }),
  "hub.week": S({
    label: "This Week",
    chrome: "experience",
    ...
    also: [
      ...
      "frontend/src/DraftHub/WeekLineupPicker.jsx",
    ],
    doNot: "Do not fork a second week hero. Extend WeeklyCommandCenter. Hero copy comes from board state: loading, error + Retry, or empty pre-draft with draft night — never No swap worth making or No bye over an error. Empty boards use the shared league empty-state (Lock a night / Link Sleeper / Sync league) — not League settings or Rate vibes. Decisions rail says Waiting on roster when there is no roster. Empty starter slots say Empty — never Waiting as a loading chip. Do not render Waiting dashes for eight empty slots on load or error. Inert slots stay flat. Decision count lives in the hero once — not also as Decisions N and a rail headline. A clear board (no bye, nobody out) uses that sentence as the lede and one quiet rail line — do not keep two muted zero counters. Refresh projections sits on the freshness line, not as the decision-panel primary. The board is a slate of editorial rows, not swap cards. Approved manual option A: position buttons open the eligible bench picker with headshots, kickoff, signed projected delta and Bench-to-slot preview; confirm before saving. Desktop right picker and mobile bottom sheet. No row-click writes. Sleeper owns linked edits. Cancel/Escape restores focus; saves announce the move. Call heat is amber Sit/Start pills; quiet rows stay unmarked — no row-wide amber wash. Start opens the Ticket sheet (week-pts delta poster; Vegas, prior PPG, def vs pos, kickoff; Private stamp only — no paper grain). Sit and start stay side by side on phone. Keep closes. Ticket Start is amber, not a second blue — hide the rail primary while the sheet is open. Wide range is a quiet marker, never a row-wide amber border or primary blue. Reserve the Start slot so P50s share a baseline. Find on an empty skill slot opens the eligible bench picker before starting a player — do not leave This Week. Empty K/DEF with no bench specialist still link to Free agents as Find K / Find DEF. Bench uses the same slate rows and spans under the rail. Name the Vibes / Vibes-adjusted projections number. Week uses the Projections stepper. Do not ship the Duel sheet or a stats-table popup. Never liturgy, prayer, altar, or bulletin in UI copy.",
  }),
  "hub.vibes": S({
    label: "Vibes",
    chrome: "experience",
    ...
    doNot: "Reuse HubExperience*. Aura is a personal start weight, not a new accent or award gold. Desktop: card left, Vibe ranking and Vibes-adjusted projections right, both in view. Front card is week-vs-vibe, not a giant cutout. Player details is the label for the profile control, neutral, not accent blue. Higher and Lower are equal; Undo is a meta control, not a third option. Sit is not amber. One progress readout lives by the card. Week and vibe week compare; aura is a labeled 0–99 meter. One rate per player per calendar day. Vibes-adjusted projections are research; This Week lineup uses the board number. Empty slots say Empty with Find {POS}. Review on This Week stays hidden until a rating exists. When Hub lineups can write, Save lineup is the one primary and Review on This Week is ghost. Otherwise Review is the one primary on the ranking card. The left column lists today's ratings or collapses. Do not scrape Wikipedia. Do not say site board. Do not mention swipe on a pointer desktop.",
  }),
  "hub.game": S({
    label: "Game center",
    chrome: "matchup",
